...
def main():
    ...
    if args.offline:
        ...
        payload=read(etf_file, {}); hydrate(root, payload.get("rows", [])); print(json.dumps({"mode":"offline_snapshot","path":str(target)})); return
    ...
    write(etf_file,{"schema_version":"historical_etf_ohlcv_v1","as_of":args.as_of,"rows":rows}); write(target/"benchmark_ohlcv.json",benchmark); write(target/"share_flow.json",{"as_of":args.as_of,"rows":[],"status":"unavailable"}); write(target/"market_calendar.json",{"as_of":args.as_of,"actual_trade_dates":sorted({r["trade_date"] for r in rows if r["trade_date"]})}); write(target/"provider_audit.json",{"as_of":args.as_of,"rows":rows+[benchmark]}); write(target/"manifest.json",manifest); hydrate(root, rows); print(json.dumps(manifest,ensure_ascii=False))
...
def hydrate(root, rows):
    ...
    with path.open("w",newline="",encoding="utf8") as f:
        ...
        for r in rows:
            ...
            w.writerow({"date":r["trade_date"],"sector_id":r.get("sector_id"),"sector_node_id":r.get("sector_node_id"),"fund_code":r["symbol"],"fund_name":r.get("fund_name_hint"),"close":r.get("close"),"pct_change":"","amount":r.get("amount") or "","turnover":"","latest_share":"","previous_share":"","share_change":"","estimated_flow":"","source_provider":r.get("source_provider"),"source_endpoint":r.get("source_endpoint"),"provider_run_id":"historical_market_input","collected_at":r.get("fetched_at"),"open":r.get("open") or "","high":r.get("high") or "","low":r.get("low") or "","volume":r.get("volume") or "","historical_input":"true"})
