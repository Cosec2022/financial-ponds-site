...
2. FP-GATE-01: keep provider-ready-but-execution-blocked reasons visible.
3. FP-WATCH-01: keep attribution conflicts sorted into daily review states.
4. FP-ATTR-01: keep cross-module conflicts visible while ETF readiness is watch-only.
5. FP-GEN-01: remove pool graph snapshot dependency from one-command runs.
6. FP-HIST-01 / FP-ROT-01: improve rotation visibility while sample history is still low.
7. FP-ETF-01 / FP-DAILY-01: keep execution decision blocked until watch-only gates are cleared.
8. FP-NEWS-01: real fixed A-share news sources.
9. FP-DATA-01: S&P 500 live provider inputs after the A-share flow path is stable.
```

## v0.10.46 Status Note
