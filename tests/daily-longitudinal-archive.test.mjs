...
const snapshot = { schema_version: "daily_longitudinal_snapshot_v1", snapshot_id: "2026-07-10:model", as_of: "2026-07-10", finality_status: "final", generated_at: "fixed", model_version: "model", row_count: 2, rows: [{ pool_id: "a" }, { pool_id: "b" }], observation_snapshot: { generated_at: "fixed", nested: [{ generated_at: "fixed" }] } };
...
test("immutable archive is idempotent, revisions changes, and rebuilds its index", async (t) => {
  ...
  const same = await appendImmutableSnapshot({ rootDir, snapshot: { ...snapshot, generated_at: "later", observation_snapshot: { generated_at: "later too", nested: [{ generated_at: "later three" }] } } });
