...
test("serves the Financial Ponds clickable pond map at the site root", async () => {
  ...
  assert.match(html, /v0\.10\.70 Observation Dashboard/);
  ...
  assert.match(html, /Review Readiness/);
  assert.match(html, /Review Analytics/);
  assert.match(html, /Major Wave/);
  assert.match(html, /Risk Gate/);
...
test("serves dashboard, general pool analysis, sector review, rotation data, module review, news review, and pond map JSON", async () => {
  ...
  assert.ok(archiveJson.candidate_state_model);
  ...
  assert.ok(archiveJson.candidate_due_review_verification);
  assert.ok(archiveJson.candidate_price_basis);
  assert.ok(archiveJson.review_readiness_report);
  assert.ok(archiveJson.candidate_review_history);
  assert.ok(archiveJson.candidate_review_analytics);
  ...
  assert.equal(eveningSummaryJson.module_id, "evening_observation_summary_v0_10_61");
  ...
  assert.equal(candidateLedgerJson.module_id, "observation_candidate_ledger_v0_10_61");
  assert.ok(candidateLedgerJson.rows.every((row) => row.boundary.includes("observe_only")));
  assert.ok(candidateLedgerJson.rows.every(hasCandidateStateFields));

  const stateModel = await worker.fetch(request("/data/candidate_state_model.json"), {});
  assert.equal(stateModel.status, 200);
  const stateModelJson = await stateModel.json();
  assert.equal(stateModelJson.module_id, "candidate_state_model_v0_10_61");
  assert.equal(stateModelJson.rows.length, candidateLedgerJson.rows.length);
  assert.ok(stateModelJson.rows.every(hasCandidateStateFields));
  ...
  assert.ok(["candidate_outcome_reviews_v0_10_64", "candidate_outcome_reviews_v0_10_65"].includes(outcomeReviewsJson.module_id));
  ...
  for (const row of outcomeReviewsJson.rows) {
    ...
    if (row.review_as_of > outcomeReviewsJson.as_of) {
      assert.equal(normalized.review_status, "pending");
      assert.equal(normalized.review_reason, "pending_not_due");
    ...
    if (normalized.review_status === "reviewed") {
      assert.ok(row.boundary.includes("observe_only"));
      assert.equal(typeof row.review_price, "number");
      assert.equal(typeof row.absolute_return, "number");
    }
  }
  ...
  assert.ok(["outcome_review_report_v0_10_64", "outcome_review_report_v0_10_65"].includes(outcomeReportJson.module_id));
  ...
  assert.notEqual("stale_data", "missing_price");
  assert.notEqual("missing_price", "missing_benchmark");
  assert.ok(dueVerificationJson.rows.filter((row) => normalizeReview(row).review_reason === "stale_data").every((row) => row.review_completed === false && row.review_price === null));
  assert.ok(dueVerificationJson.rows.filter((row) => normalizeReview(row).review_reason === "missing_price").every((row) => row.review_price === null));
  assert.ok(dueVerificationJson.rows.filter((row) => normalizeReview(row).review_reason === "missing_benchmark").every((row) => typeof row.review_price === "number" && row.benchmark_return === null && (row.outcome_available ?? false) === false));

  const priceBasis = await worker.fetch(request("/data/candidate_price_basis.json"), {});
  assert.equal(priceBasis.status, 200);
  const priceBasisJson = await priceBasis.json();
  assert.equal(priceBasisJson.module_id, "candidate_price_basis_v0_10_61");
  assert.ok(priceBasisJson.rows.length >= candidateLedgerJson.rows.length);
  assert.ok(priceBasisJson.rows.every((row) => row.boundary.includes("observe_only")));
  assert.ok(priceBasisJson.rows.every(hasCandidateStateFields));

  const reviewReadiness = await worker.fetch(request("/data/review_readiness_report.json"), {});
  assert.equal(reviewReadiness.status, 200);
  const reviewReadinessJson = await reviewReadiness.json();
  assert.equal(reviewReadinessJson.module_id, "review_readiness_report_v0_10_61");
  assert.equal(reviewReadinessJson.baseline_available_count + reviewReadinessJson.baseline_missing_count, reviewReadinessJson.next_due_candidate_count);
  ...
  assert.ok(["candidate_review_history_v0_10_64", "candidate_review_history_v0_10_65"].includes(reviewHistoryJson.module_id));
  ...
  assert.ok(reviewHistoryJson.rows.some((row) => row.t1_review_result.unavailable_reason || ["reviewed", "pending"].includes(normalizeReview(row.t1_review_result).review_status)));
  ...
  assert.ok(["candidate_review_analytics_v0_10_64", "candidate_review_analytics_v0_10_65"].includes(reviewAnalyticsJson.module_id));
...
function hasCandidateStateFields(row) {
  const states = new Set(["Noise", "Pulse", "Early Right", "Major Candidate", "Confirmed Trend", "Overheated", "Cooling", "Failed"]);
  const gates = new Set(["pass", "caution", "block", "insufficient_data"]);
  const overheatOk = typeof row.overheat_score === "number" || row.overheat_score === "unavailable";
  const majorOk = typeof row.major_wave_score === "number" || row.major_wave_score === "unavailable";
  return states.has(row.candidate_state)
    && overheatOk
    && majorOk
    && gates.has(row.risk_gate_status)
...
function normalizeReview(row) {
  if (["pending", "reviewed", "unavailable", "skipped"].includes(row.review_status)) return row;
  const legacy = {
    pending_not_due: { review_status: "pending", review_reason: "pending_not_due" },
    reviewed: { review_status: "reviewed", review_reason: null },
    unavailable_market_closed: { review_status: "unavailable", review_reason: "awaiting_eod_data" },
    unavailable_data_stale: { review_status: "unavailable", review_reason: "stale_data" },
    unavailable_missing_price: { review_status: "unavailable", review_reason: "missing_price" },
    unavailable_missing_benchmark: { review_status: "unavailable", review_reason: "missing_benchmark" },
    skipped_invalid_baseline: { review_status: "skipped", review_reason: "invalid_baseline" }
  };
